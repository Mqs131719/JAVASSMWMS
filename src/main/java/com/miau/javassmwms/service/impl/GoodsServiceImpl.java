...
import com.miau.javassmwms.util.ThreadPoolSingle;
...
//        System.out.println(goods1);
    ...
    public R upload2(MultipartFile file) throws IOException {
        ...
        if (!file.isEmpty()) {
            //2.解析Excel -EasyExcel
            //2.解析Excel数据
            List<GoodsExcelDto> list = new ArrayList<>();
            EasyExcel.read(file.getInputStream(), GoodsExcelDto.class, new ReadListener<GoodsExcelDto>() {
                @Override
                public void invoke(GoodsExcelDto o, AnalysisContext analysisContext) {
                    list.add(o);
                    if (list.size() >= 2000) {
                        List<GoodsExcelDto> list1 = new ArrayList<>(list);
                        list.clear();
                        ThreadPoolSingle.getInstance().executor.execute(() -> {
                            dao.saveBatch(list1);
                        });
                    }
                }
